...
every day, and that error response was being stored as a row (an existing DB
had 3651 such rows). The query now requests the current ``metrics`` shape, and
GraphQL error-only responses are never stored.
"""
