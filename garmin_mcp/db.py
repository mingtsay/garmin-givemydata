...
def migrate_sync_log_table(conn: sqlite3.Connection) -> None:
    """Record the date range each sync covered so incremental runs can anchor on
    coverage rather than inferring it from ``MAX(calendar_date)`` (which lags
    once empty placeholder days are no longer written)."""
    _add_columns(conn, "sync_log", [("data_start", "TEXT"), ("data_end", "TEXT")])


def init_db(conn: sqlite3.Connection) -> None:
    """Create all tables and indexes if they do not already exist."""
    conn.executescript(_SCHEMA_SQL)
    migrate_sync_log_table(conn)
